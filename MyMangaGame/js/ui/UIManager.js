...
/**
    ...
    updateMangaPanel(state) {
        ...
        // 3. 构建 HTML 内容
        let html = `
            <h3 style="margin-bottom:8px; font-size:16px;">🏆 业内排名</h3>
            ...
            <h3 style="margin-bottom:8px; font-size:16px;">📖 连载状态</h3>
        `;
        ...
        if (career.currentWork) {
            ...
            html += `
                <div style="background:#FFF0F5; padding:10px; border:2px solid #FF69B4; border-radius:4px; position:relative;">
        ...
        if (career.history && career.history.length > 0) {
            html += `<h3 style="margin:15px 0 8px 0; font-size:16px;">📜 出版履历</h3>`;
